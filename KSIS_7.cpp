...
#define IP_DEST_PROT_UNREACHABLE 11005
...
void Ping(const char* cHost, unsigned int Timeout, unsigned int RequestCount)
{
	...
	unsigned int MaxMS = 0;				//максимально время ответа(мс)
	...
	for (unsigned int c = 0; c < RequestCount; c++) {
		int dwStatus = IcmpSendEcho(hIP, ipaddr, SendData, sizeof(SendData),
			&option, pIpe, sizeof(ICMP_ECHO_REPLY) + sizeof(SendData), Timeout);
		if (dwStatus > 0)
		{
			for (int i = 0; i < dwStatus; i++)
			{
				...
				cout << "Ответ от  " << (int)*(pIpPtr)
					<< "." << (int)*(pIpPtr + 1) << "." << (int)*(pIpPtr + 2)
					<< "." << (int)*(pIpPtr + 3) << ": число байт = " << pIpe->DataSize
					<< " время = " << pIpe->RoundTripTime << "мс TTL = " << (int)pIpe->Options.Ttl;
			...
			if (pIpe->Status)
			{
				...
				case IP_DEST_PORT_UNREACHABLE:       cout << "Remote host may be down." << endl;
					break;
				case IP_REQ_TIMED_OUT:       cout << "Request timed out." << endl;
					break;
				case IP_TTL_EXPIRED_TRANSIT:       cout << "TTL expired in transit." << endl;
					break;
				default:       cout << "Error code: %ld" << pIpe->Status << endl;
					break;
				}
			}
		}
	...
	cout << "Статистика Ping	"
		<< iByte << "." << iByte + 1 << '.' << iByte + 2 << '.' << iByte + 3 << endl;
